    ...
    <main className="container-narrow py-16 md:py-24">
      <header className="mb-10 max-w-2xl">
        ...
        <h1 className="mt-3 font-display text-5xl text-moss-deep md:text-6xl">
          {group === ALL ? t("shop_title_all") : group}
        </h1>
        <p className="mt-5 text-base text-muted-foreground">
          {t("shop_subtitle")}
          {group !== ALL && ` ${t("shop_browse_collection")} ${group.toLowerCase()} ${t("shop_collection_below")}`}
        </p>
      </header>
      ...
      <div className="mb-6">
        <div className="relative max-w-md">
          ...
          {query && (
            <button
              onClick={() => setQuery("")}
              aria-label={t("shop_clear_search")}
      ...
      <div className="mb-10 flex flex-wrap gap-2 border-y border-border py-4">
        {categories.map((c) => {
          ...
          return (
            ...
            >
              {c === "All" ? t("shop_all") : c} <span className="ml-1 opacity-60">{count}</span>
            </button>
          );
        })}
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="border border-dashed border-border py-20 text-center">
          <p className="font-display text-2xl text-moss-deep">{t("shop_no_match")}</p>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("shop_no_match_body")}
          </p>
          <button
