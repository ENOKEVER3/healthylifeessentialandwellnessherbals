      ...
      <main className="container-narrow py-16 md:py-20">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-5 text-muted-foreground">
            {t("checkout_order_label")} <span className="font-medium text-foreground">{snapshot.orderNumber}</span> {t("checkout_received_a")} <span className="font-medium text-foreground">{snapshot.email}</span>.
          </p>
        </div>

        <div className="mx-auto mt-14 max-w-2xl">
          <p className="mb-3 inline-flex items-center gap-2 text-xs uppercase tracking-[0.22em] text-muted-foreground">
            <Mail className="h-3.5 w-3.5" strokeWidth={1.6} /> {t("checkout_email_preview")}
          </p>
          <div className="overflow-hidden border border-border bg-cream/40 shadow-soft">
            <div className="flex items-center justify-between border-b border-border bg-background/80 px-5 py-3 text-xs">
              ...
              <span className="text-muted-foreground">{t("checkout_inbox")} · {snapshot.date}</span>
            </div>
            <div className="border-b border-border bg-background px-6 py-4 text-sm">
              <p><span className="text-muted-foreground">{t("checkout_from")}</span> Healthy Life Essentials &lt;orders@healthylifeessentials.co&gt;</p>
              <p><span className="text-muted-foreground">{t("checkout_to")}</span> {snapshot.email}</p>
              <p><span className="text-muted-foreground">{t("checkout_subject")}</span> {t("checkout_subject_value").replace("{{order}}", snapshot.orderNumber)}</p>
            </div>
            ...
            <div className="bg-background px-7 py-10 md:px-10 md:py-12">
              ...
              <p className="mt-8 border-t border-border pt-5 text-[11px] leading-relaxed text-muted-foreground">
                {t("checkout_made_with_care")}<br />
                {t("checkout_demo_email")}
              </p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          <section>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input id="email" name="email" type="email" required placeholder="you@garden.co" className="mt-1.5" />
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div><Label htmlFor="first">{t("checkout_first_name")}</Label><Input id="first" name="first" required className="mt-1.5" /></div>
              <div><Label htmlFor="last">{t("checkout_last_name")}</Label><Input id="last" name="last" required className="mt-1.5" /></div>
              <div className="sm:col-span-2"><Label htmlFor="addr">{t("checkout_address")}</Label><Input id="addr" required className="mt-1.5" /></div>
              <div><Label htmlFor="city">{t("checkout_city")}</Label><Input id="city" required className="mt-1.5" /></div>
              <div><Label htmlFor="zip">{t("checkout_postal")}</Label><Input id="zip" required className="mt-1.5" /></div>
            </div>
          </section>
          ...
          <section>
            ...
            <p className="mb-5 text-xs italic text-muted-foreground">
              {t("checkout_payment_demo")}
            </p>
            <div className="space-y-4">
              <div><Label htmlFor="card">{t("checkout_card_number")}</Label><Input id="card" placeholder="4242 4242 4242 4242" className="mt-1.5" /></div>
              <div className="grid grid-cols-2 gap-4">
                <div><Label htmlFor="exp">{t("checkout_expiry")}</Label><Input id="exp" placeholder="MM / YY" className="mt-1.5" /></div>
                <div><Label htmlFor="cvc">{t("checkout_cvc")}</Label><Input id="cvc" placeholder="123" className="mt-1.5" /></div>
              </div>
            </div>
          </section>
          ...
          >
            {t("checkout_confirm_order")} {formatNGN(total)}
          </Button>
        </form>
